...
inline void parallel_cpu_moe_gemv(
    ...
    pool.parallel_for(0, batch_size, [&](size_t /*worker_id*/, size_t start_b, size_t end_b) {
        for (size_t b = start_b; b < end_b; ++b) {
            const float* in_vec = input + (b * in_features);
            float* out_vec = output + (b * out_features);
            ...
            for (int k = 0; k < top_k; ++k) {
                int exp_idx = expert_indices[b * top_k + k];
                float gate_w = routing_weights[b * top_k + k];

                const float* w_exp = expert_weights + (exp_idx * out_features * in_features);

                for (int oc = 0; oc < out_features; ++oc) {
                    const float* w_row = w_exp + (oc * in_features);
                    float acc = 0.0f;
